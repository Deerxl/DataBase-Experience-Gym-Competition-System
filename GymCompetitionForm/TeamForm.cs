...
namespace GymCompetitionForm
{
    public partial class TeamForm : Form
    {
        ...
        {
            this.team = team;
            label1.Text = team.Name + ", 欢迎！";
        }

        private void TeamForm_Load(object sender, EventArgs e)
        {
            leaderBindingSource.DataSource = service.QueryLeadersByTeamAccout(team.Account);
            doctorBindingSource.DataSource = service.QueryDoctorsByTeamAccount(team.Account);
            athleteBindingSource.DataSource = service.QueryAthletesByTeamAccount(team.Account);
            gameInformationBindingSource.DataSource = service.QueryGameInformationById();
            coachBindingSource.DataSource = service.QueryCoachByTeamAccount(team.Account);
            refereeBindingSource.DataSource = service.QueryRefereeByTeamAccount(team.Account);
